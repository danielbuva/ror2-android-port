...
if sh('pm path '+package):raise RuntimeError('Smoke package already exists: inspect its ownership before replacing/removing it.')
...
installed=False
try:
    result['install']=cmd(base+['install','-r',str(apk)]); installed=True
    ...
    if installed:
        ...
        result['removed']=not bool(sh('pm path '+package))
