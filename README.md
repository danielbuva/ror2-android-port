...
Manual device use: the launcher name is **RoR2 Porting Lab**. Keep the owned installation available for the user and update it in place with the existing install/sync workflow; unchanged payload files are skipped. Scene/smoke experiments preserve a pre-existing owned installation. Do not run cleanup/reset merely to finish a normal iteration. A deliberate reset must first preserve compatible payload/profile evidence and follow the existing ownership safeguards. The current app is a laboratory probe, not a playable game.
