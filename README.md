...
Tool wrappers: `scripts/assetripper`, `scripts/ilspycmd`. MCP server: `scripts/mcp-server.sh` (loopback port 8080); `scripts/open-smoke.sh` opens the smoke editor and connects it. Close that editor before a batch build. Probe with `.local/tools/unity-mcp/Server/.venv/bin/python scripts/mcp-check.py`.
