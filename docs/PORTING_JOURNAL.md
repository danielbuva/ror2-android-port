...
## J83 — Recovered lifecycle probe compile errors retained

Attempt work/experiments/scene-runtime/20260920T003029.044943Z stopped before device execution. The authored probe used ambiguous Path and accessed NetworkStateMachine.stateMachines as public. Current original source declares that serialized array private, unlike the pinned community project's accessible API. Correct to System.IO.Path and read the exact private field diagnostically; no game assembly or serialized relationship changes. Failed stage, compiler lines, build receipt and contract retained. This is not evidence against recovered body/master lifecycle capability.

## J84 — Recovered Commando and PlayerMaster setup pass independently

Attempt work/experiments/scene-runtime/20260920T003228.271329Z corrects J83's authored API references. Prior art: pinned Starstorm2 SS2VanillaSurvivor a9a4baddc5dd4405e893ab5dfc684eb9e27c26f8 preserves named state-machine/network/death/hurt relationships; pinned DebugToolkit d1e2f0aa4b8ac4747547db0fcd87344953432f06 distinguishes actual server spawning. Current original CharacterBody/CharacterMaster/BuffCatalog source defines the tested contract. PlayerMaster is the recovered player prefab, not CommandoMonsterMaster. Its root alone is made inactive before import, as already done for CommandoBody.

- S33: original SetBuffDefs initializes an empty diagnostic catalog; original buffer allocation returns separate zero-length arrays. Prior catalog array identities restore. 12 assertions, 26.15 seconds.
- S34: recovered Commando instantiated inactive with no missing components. Original CharacterBody.Awake invokes one global awake event and caches actual identity, motor, input, health and skill components. Model/hurtbox/main-hurtbox/core references, capsule radius, buff storage and serialized network state-machine membership agree. 261 assertions including component enumeration, 26.11 seconds.
- S35: separate recovered-body launch repeats that setup, then original OnEnable/OnDisable add/remove exactly the body from the original registry while the root remains inactive. 263 assertions including component enumeration, 26.06 seconds.
- S36: recovered inactive PlayerMaster has original Inventory allocation and CharacterMaster.Awake. Actual inventory, identity and player-controller caches agree; no body is spawned. Original master registration/unregistration succeeds, followed by original master OnDestroy and inventory queued disposal. 21 assertions, 26.10 seconds.

Four launches pass 557 assertions, most of which are repeated non-null recovered-component checks; 20 host tests pass. All 45 original DLL hashes unchanged. APK SHA-256 b8912a26ab7ed6708bcbd329209f1d912abfeb5e169232c2832ecc9cbb9f5c06. Current-process errors are only the inherited Windows shader bundle rejection; no matched-process crash. Four separate checkpoint receipts preserve original source/build/payload/isolation/contract evidence. Owned app stopped, selector removed, transient stage archived; compile failure remains preserved.

Limits: methods are invoked selectively on inactive recovered prefabs. Empty buff catalog is diagnostic, not stock buff initialization. No Body.Start/Master.Start, RecalculateStats, actual master/body network link, client/player semantics, state ticking on this recovered body or full teardown. Body.OnDestroy includes native audio/master callbacks and is not invoked; only the fixture-created model subscription is detached. No original gameplay method is replaced. Static link audit identifies inventory behavior, four direct Lunar item references and the null-equipment/absent-QuestVolatileBattery equality hazard before a future body-master getter probe. Full L5.5 remains open.

## J85 — Recovered network spawning and master-ID recording pass

Attempt work/experiments/scene-runtime/20260920T004148.919501Z uses J84 recovered inactive prefabs and diagnostic buff setup. Consulted community-prior-art and pinned DebugToolkit d1e2f0aa4b8ac4747547db0fcd87344953432f06 actual server spawning, plus Starstorm2 a9a4baddc5dd4405e893ab5dfc684eb9e27c26f8 state-machine wiring. Exact original NetworkStateMachine, NetworkIdentity/NetworkServer, Util and body/master methods determine the tests. No authority flags or cached body/master pointers are assigned.

- S37: original NetworkStateMachine.Awake assigns the expected index, networker and original NetworkIdentity to every serialized recovered sibling machine. 264 assertions including prefab scans, 25.12 seconds. This is setup, not state replication.
- S38: actual loopback server Spawn registers inactive recovered CommandoBody with a nonzero ID; real registry lookup returns the same object. Original body UpdateAuthority and Util.HasEffectiveAuthority pass. Owned UnSpawn removes the registry entry. 268 assertions, 25.11 seconds.
- S39: independent recovered PlayerMaster Spawn/registry/authority/UnSpawn path passes, retaining its actual inventory and player controller. 28 assertions, 26.14 seconds.
- S40: both recovered objects receive distinct actual server IDs. The original public body.masterObject setter records the master's ID; GetMasterObjectId and actual NetworkServer.FindLocalObject agree. Body inventory and master's reciprocal body relationship remain unset, as asserted. 278 assertions, 26.07 seconds. The inventory-adopting master getter is intentionally not invoked.

Four launches pass 838 assertions, predominantly repeated recovered-component scans; 20 host tests pass. All 45 original DLL hashes unchanged. APK SHA-256 58131f964f8dc8b29ecaa7beb6f25ef9dae0a211ba2209e2baaf8dcd304a6146. Per-process logs contain only the inherited Windows shader rejection; no matched-process crashes. Four distinct checkpoint receipts retain stage/build/payload/network-contract evidence. Owned servers shut down, app stopped, selector removed, transient stage archived.

These results establish actual network identity prerequisites on recovered objects, not full master/body linkage, a local player/client, replicated states or gameplay activation. The next original getter adopts inventory and enters measured item/equipment/skill callbacks. Located the four required Lunar ItemDefs and QuestVolatileBattery EquipmentDef in current input; SkillDefs with duplicate basenames require exact path/type selection. No content binding or callback success is inferred from those static locations. Preserve formal L5.5 and Start/stat gates.


## J86 — Adoption build infrastructure failures retained

Attempt work/experiments/scene-runtime/20260920T005206.336903Z prepares the five measured definitions successfully. Initial build dispatch fails because the local pinned MCP server is absent. After restarting it, the isolated editor requires reconnection. A manual editor restart omitted the established isolated Android environment, so compilation succeeds but Gradle packaging cannot read the host default debug keystore. No device launch or game-code capability result follows.

The failed terminal receipt, editor/build logs and staged source snapshot are retained under packaging-failure; the first connection failure is retained separately. Restart using the existing `./dev editor --target lab --action open`, which supplies the correct isolated Android and Gradle directories. Rebuild the unchanged probe after live preflight; no key changes, cache clearing, middleware changes or original-game modifications are justified.

## J87 — Original recovered inventory adoption passes on Android

Accepted unchanged-probe rebuild in work/experiments/scene-runtime/20260920T005206.336903Z corrects only the J86 editor environment. Pinned DebugToolkit actual server spawning and current original CharacterBody/ItemCatalog/EquipmentCatalog sources inform the experiment. Closure retains 68 source files; exact typed paths distinguish Lunar ItemDefs from same-named SkillDefs. Original catalog methods index four recovered Lunar items and QuestVolatileBattery equipment; the harness binds only those measured content fields.

- S41: all five typed definitions and original catalog identities pass, with empty equipment remaining absent. 24 assertions, 25.14 seconds.
- S42: original recovered body getter resolves the actual spawned PlayerMaster, adopts its real Inventory and identifies its player-controller component. Original OnInventoryChanged completes and emits exactly one observed callback. Repeated getter reads retain identity without another callback; empty equipment creates no quest behavior. Reciprocal master body remains absent, as asserted. 302 assertions including inherited recovered-component scans, 25.24 seconds.

Two separate fresh processes pass. Twenty host tests pass; all 45 original DLL hashes unchanged. APK SHA-256 1a484ef54b1fab1eb0075f0c63b0850cfcc82691333b8b2f8e72cf79996e1410. Current-process logs contain only the five inherited Windows shader rejection lines; no matched-process crashes. Adoption screenshot reviewed: foreground Vulkan lab fixture, no visual gameplay claim. Owned app stopped, selector removed, transient stage archived. RECOVERED_ADOPTION_CONTENT and RECOVERED_INVENTORY_ADOPTION receipts preserve separate assertions and shared build/payload; J86 failures remain retained.

Limits: explicitly empty inventory, four-item/one-equipment catalog, empty buff storage and inactive recovered roots. Optional absent definitions use original zero-count behavior. No callback suppression or cached-reference assignments; cleanup detaches the fixture-created inventory subscription before original master/inventory disposal. This does not establish normal catalog loading, item grants, ongoing inventory-event propagation, normal stats, reciprocal linkage, local client or full L5.5.

Next isolate actual stat prerequisites before broader linking. Current RecalculateStats immediately returns without Run.instance, so a no-exception call with absent Run is not success. Its next dependencies include actual TeamManager team experience/level and initialized body.teamComponent. CharacterMaster.SpawnBody additionally depends on Awake-populated component references, loadout, Run spawn callbacks and network lifecycle. Do not force reciprocal caches or activate the full prefab to skip these boundaries.

## J88 — Three stat prerequisites pass; original skill initialization missing

Attempt work/experiments/scene-runtime/20260920T011301.877114Z retains four separate Android launches. S43 original TeamManager singleton/Start, level-one initialization, 20-experience level-two threshold and reset pass (13 assertions). S44 recovered TeamComponent.Awake, player-team membership, hurtbox propagation and original removal pass (267 assertions including inherited component scans). S45 original Run.OnEnable/OnDisable singleton lifecycle passes while inactive (3 assertions); this does not execute Run.Awake/Start or initialize a game.

S46 fails in original GenericSkill.get_baseRechargeInterval through flatCooldownReduction during RecalculateStats. The recovered slots have not run GenericSkill.Awake, which binds the actual default SkillDef and selects its state machine. No stat completion callback is reached; no base-stat capability is accepted. Preserve the full failed report/build/stage. All 45 original DLLs unchanged; 20 host tests pass. Per-process logs contain only inherited Windows shader rejection; no matched crash.

Next candidate invokes original GenericSkill.Awake on the recovered slots and checks actual SkillFamily default identity, body/state-machine references and finite cooldowns in an independent probe, then retries the unchanged stat calculation. Pinned Starstorm2 SS2VanillaSurvivor skill-family variants/state-machine wiring supports preserving these relationships; current GenericSkill source specifies initialization and original OnDestroy cleanup. No direct skillDef assignment, cooldown substitution or callback suppression.

## J89 — Recovered skill setup passes; stat calculation reaches motor bounds

Attempt work/experiments/scene-runtime/20260920T012057.588196Z independently passes S47: all four original GenericSkill.Awake calls select the recovered family's default SkillDef, resolve the original body/state-machine relationships and compute finite cooldowns (278 assertions including inherited scans). Original unassignment cleanup completes. Twenty host tests pass and all 45 original DLLs remain unchanged.

S46 now reaches UpdateAllTemporaryVisualEffects, then fails in CharacterMotor.capsuleHeight through CharacterBody.bestFitRadius. This differs from J88: skill setup is proven and the calculation has progressed to a missing motor Awake cache. Completion is still not accepted. No unexplained process errors or matched crashes accompany either launch; both failed stat attempts remain immutable.

Reassessed hypothesis: selective recovered lifecycle initialization remains incomplete, not evidence of an AOT or formula failure. Original CharacterMotor.Awake supplies the capsule reference and initial dimensions; it was previously exercised only on diagnostic motor fixtures. Next isolate that same original method on the recovered component, verify dimensions/effect bounds against the actual collider, then retry stats. No solver movement or full motor Start is claimed. Original OnDestroy returns the collection allocated by Awake.

## J90 — Recovered motor capsule passes; stat effect handler needs actual buff definitions

Attempt work/experiments/scene-runtime/20260920T012614.284906Z passes S48: original recovered CharacterMotor.Awake supplies actual capsule height/radius and initial dimensions; original CharacterBody.bestFitRadius agrees with the collider. Original motor OnDestroy returns its pooled collection. 264 assertions including inherited prefab checks; 25.15 seconds. No motor Start, solver movement or complete recovered-body movement claim.

S46 advances past that boundary but fails in UpdateKnockBackHitVisualsAndGracePeriod, reached through UpdateAllTemporaryVisualEffects. Current source directly reads DLC2Content.Buffs.KnockUpHitEnemies.buffIndex and KnockUpHitEnemiesJuggleCount.buffIndex. Both definitions are absent from the intentionally empty buff catalog. This is a measured content dependency, not an optional null-safe buff query. Stat completion callback remains unobserved; do not accept partially calculated values or create a stats checkpoint.

Located both original BuffDefs in the recovered KnockBackHitEnemies item directory. Next separately prove their typed identities, original catalog indices and zero counts; initialize the diagnostic catalog before CharacterBody.Awake allocates its buffers, then rerun the original handler/stats without replacing or skipping it. Restore catalog name mappings as well as arrays when this becomes a populated catalog. The handler also queries motor grounding; inspect its actual authority/solver branch before the next run. Do not jump to full character activation.

Eight sequential fresh Android launches across J88–J90 yield five accepted prerequisites and three preserved stat failures at distinct boundaries. Each process survives over 25 seconds; 20 host tests pass, all 45 original DLLs unchanged. Per-process logs show only inherited Windows shader rejection, with no matched crashes. Final APK SHA-256 46dbe20e9b751e4a7061f704852a3aa59a71f054582f5d5888bd76c391a3ae85. Separate TEAM_EXPERIENCE_CONTEXT, RECOVERED_TEAM_MEMBERSHIP, RUN_SINGLETON_LIFECYCLE, RECOVERED_SKILL_SETUP and RECOVERED_MOTOR_CAPSULE checkpoints retain their own builds. Owned app stopped, selectors removed and all transient stages archived. Full Run initialization, stat completion, reciprocal linkage and L5.5 remain open.
