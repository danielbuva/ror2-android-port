...
J62 review: unavailable platform access affects both C and A; no independent C failure currently warrants source reconstruction. Retain C and select L10-a static no-audio lifecycle attribution, with runtime design returned to Astra. J55 is only an early guard; later native query/dialog, bank wait and shutdown safety remain unproven. Doctor currently lacks a registered editor; restore and verify before runtime work.
